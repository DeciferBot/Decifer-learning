"""
Full batch generation for Year 6 (KS2) Maths, English, and Science.
...
Prerequisites (must run first):
  npx tsx --env-file=.env.local scripts/seed-year6.ts
  npx tsx --env-file=.env.local scripts/seed-chunks-maths-y6.ts
  npx tsx --env-file=.env.local scripts/seed-chunks-english-y6.ts
  npx tsx --env-file=.env.local scripts/seed-chunks-science-y6.ts
  python3 /tmp/embed_chunks.py   (on DO droplet, to compute embeddings for new chunks)

Run: /root/pipeline-venv/bin/python3 scripts/generate-batch-y6.py
"""
...
log = logging.getLogger("generate-batch-y6")
...
Y6_MATHS_TOPICS = [
    "y6-maths-place-value-rounding",
    "y6-maths-fractions-decimals-percentages",
    "y6-maths-algebra-expressions",
    "y6-maths-ratio-proportion",
    "y6-maths-geometry-shapes",
    "y6-maths-statistics",
]

Y6_ENGLISH_TOPICS = [
    "y6-english-grammar-subjunctive-passive",
    "y6-english-punctuation-advanced",
    "y6-english-vocabulary-etymology",
    "y6-english-reading-inference",
    "y6-english-writing-narrative",
]

Y6_SCIENCE_TOPICS = [
    "y6-science-evolution-inheritance",
    "y6-science-living-things-classification",
    "y6-science-circulatory-system",
    "y6-science-light",
]
...
def main():
    all_slugs = Y6_MATHS_TOPICS + Y6_ENGLISH_TOPICS + Y6_SCIENCE_TOPICS
    print(f"\n{'='*72}")
    print(f"  Y6 Full Batch Generation — {datetime.now().strftime('%Y-%m-%d %H:%M')}")
    ...
    if missing:
        ...
        log.error("Run: npx tsx --env-file=.env.local scripts/seed-year6.ts")
        sys.exit(1)

    log.info(f"All {len(topics)} topics verified in DB ✓")

    # Check Y6 chunks have embeddings
    import psycopg2
    from pgvector.psycopg2 import register_vector
    conn = psycopg2.connect(config.DATABASE_URL)
    register_vector(conn)
    with conn.cursor() as cur:
        cur.execute("SELECT COUNT(*) FROM curriculum_chunks WHERE year_group='year-6' AND embedding IS NULL")
        null_count = cur.fetchone()[0]
        cur.execute("SELECT COUNT(*) FROM curriculum_chunks WHERE year_group='year-6'")
        total_count = cur.fetchone()[0]
    conn.close()

    if total_count == 0:
        log.error("No Y6 curriculum_chunks found. Run the seed-chunks-*-y6.ts scripts first.")
        sys.exit(1)
    if null_count > 0:
        log.error(f"{null_count}/{total_count} Y6 chunks have NULL embeddings. Run embed_chunks.py first.")
        sys.exit(1)
    log.info(f"Y6 chunks: {total_count} total, all have embeddings ✓")
...
if __name__ == "__main__":
    ...
    try:
        with pipeline_lock("batch-year-6"):
            main()
