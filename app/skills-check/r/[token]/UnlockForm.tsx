...
export function UnlockForm({ token, year, subject }: Props) {
  ...
  async function submit(e: React.FormEvent) {
    ...
    setError(null)
    try {
      const res = await fetch('/api/skills-check/unlock', {
        method: 'POST',
      ...
      // Re-render the page from the server, which now returns the full report.
      router.refresh()
    } catch (err) {
  ...
  return (
    <section className="rounded-2xl border border-ember/20 bg-brand-50 p-5">
      ...
      <ul className="mt-3 space-y-1.5 text-[15px] leading-relaxed text-ink-2">
        ...
        <li>A copy you can keep, and a reminder to check again in six weeks.</li>
      </ul>
      ...
      <p id="parent-email-note" className="mt-3 text-xs leading-relaxed text-ink-2">
        ...
        the six-week reminder, and every email has a one-click link to delete it. We hold nothing
