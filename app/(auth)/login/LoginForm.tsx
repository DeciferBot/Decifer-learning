      ...
      try {
        ...
        const { error: otpError } = await supabase.auth.signInWithOtp({
          ...
          options: {
            ...
            // server-side and the session cookie is set correctly.
    ...
    <div className="mt-5">
      ...
      {mode === 'password' && (
        <form onSubmit={handlePasswordSubmit} className="space-y-4" noValidate>
          ...
          <p className="text-center text-sm text-muted">
            ...
            <a href="/reset-password" className="font-semibold text-maths underline">
              Reset it
            </a>
          </p>
        </form>
      ...
      {mode === 'magic' && (
        <form onSubmit={handleMagicSubmit} className="space-y-4" noValidate>
          {magicSent ? (
            <div className="rounded-md bg-correct/10 px-3 py-4 text-sm text-correct text-center">
