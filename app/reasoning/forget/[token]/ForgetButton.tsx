...
export function ForgetButton({ token, action }: Props) {
  ...
  if (state === 'deleted') {
    return (
      <p
        role="status"
        className="rounded-2xl border border-success/30 bg-success/10 p-5 text-[15px] text-ink"
      >
  ...
  if (state === 'nothing') {
    return (
      <p
        role="status"
        className="rounded-2xl border border-black/5 bg-surface p-5 text-[15px] text-ink-2"
      >
