...
//
// quiz_questions.difficulty_b is null for every question in the catalogue and
// will stay that way: item calibration needs 20 first-attempt responses each,
// across 6,971 questions, and the product has around a thousand answers in
// total. With every difficulty null, rankByTargetDifficulty treats all items as
// equally distant and its ordering collapses back to the shuffle.
//
// question_difficulty_priors holds the same responses pooled by
// (question_type, tier), which does reach a usable sample. An item falls back to
// its own calibration first, then its type-and-tier prior, then the tier-only
// prior stored under question_type '*'. Nightly writer: /api/cron/calibrate-difficulty.

type PriorMap = Map<string, number>

const priorKey = (questionType: string, tier: string) => `${questionType}\u0000${tier}`

async function loadDifficultyPriors(supabase: SupabaseClient): Promise<PriorMap> {
  const { data, error } = await supabase
    .from('question_difficulty_priors')
    .select('question_type, tier, difficulty_b')

  const map: PriorMap = new Map()
  if (error || !data) return map
  for (const row of data as Array<{ question_type: string; tier: string; difficulty_b: number }>) {
    map.set(priorKey(row.question_type, row.tier), row.difficulty_b)
  }
  return map
}

/** An item's best available difficulty: its own, then its group's, then its tier's. */
function effectiveDifficulty(q: AdaptiveQuestion, priors: PriorMap): number | null {
  if (q.difficulty_b != null) return q.difficulty_b
  return (
    priors.get(priorKey(q.question_type, q.tier)) ??
    priors.get(priorKey('*', q.tier)) ??
    null
  )
}

// Estimate a child's ability (θ) on this topic from their recent answers and the
// best available difficulty of the questions they answered. Items with no
// difficulty at all contribute to the success rate but not the difficulty anchor
// (see lib/irt.ts).
function estimateAbility(
  ...
  const diffById = new Map(pool.map((q) => [q.id, effectiveDifficulty(q, priors)]))
...
// Two mixes:
//
//   balanced   ~40% sprout, ~40% explorer, ~20% lightning. The standard quiz.
//   confidence ~80% sprout, ~20% explorer, no lightning, ordered easiest first.
//              Used for a child's FIRST attempt on a topic.
//
// WHY confidence exists: measured on real answers, sprout runs at 63% correct,
// explorer at 49% and lightning at 54%. Against the balanced mix that predicts an
// average score of 0.4(63) + 0.4(49) + 0.2(54) = 55%, and the pass mark is 70%.
// The observed average across 99 completed attempts was 59%, with 60 of them
// failing. A child's first meeting with a topic should not be a coin flip.
//
// Fills any shortfall from the remaining pool so the target count is always met
// if the pool allows.

export type QuizMix = 'balanced' | 'confidence'

const MIX_WEIGHTS: Record<QuizMix, Record<QuestionTier, number>> = {
  balanced:   { sprout: 0.4, explorer: 0.4, lightning: 0.2 },
  confidence: { sprout: 0.8, explorer: 0.2, lightning: 0 },
}

/** Easiest first. Used to order a confidence quiz and to fill its shortfalls. */
const TIER_ORDER: QuestionTier[] = ['sprout', 'explorer', 'lightning']

function byTierAscending(items: AdaptiveQuestion[]): AdaptiveQuestion[] {
...
function pickWithTierBalance(
  ...
  count: number,
  ability?: number,
  mix: QuizMix = 'balanced',
  priors: PriorMap = new Map(),
): AdaptiveQuestion[] {
  // When an ability estimate is supplied, prefer items pitched near it WITHIN
  // each tier (desirable difficulty), using each item's best available
  // difficulty. Shuffling first keeps variety among equal-distance items, so a
  // pool with nothing to rank on degrades to the original shuffle behaviour.
  // Tier balance is unchanged — formal tier gates are not bypassed (CLAUDE.md §10).
  // Ranked through a wrapper so the questions handed to the child keep their own
  // difficulty_b (usually null) rather than being overwritten with a group prior.
  const order = (items: AdaptiveQuestion[]) =>
    ability == null
      ? shuffle(items)
      : rankByTargetDifficulty(
          shuffle(items).map((q) => ({ item: q, difficulty_b: effectiveDifficulty(q, priors) })),
          ability,
        ).map((x) => x.item)
  ...
  const targets: Record<QuestionTier, number> = {
    sprout: Math.ceil(count * weights.sprout),
    explorer: Math.ceil(count * weights.explorer),
    lightning: Math.floor(count * weights.lightning),
  }
  ...
  // Fill any shortfall from any remaining questions. A confidence quiz fills from
  // the easiest tier up, so a topic thin on sprout borrows explorer before it
  // ever reaches for lightning.
  if (selected.length < count) {
    const usedIds = new Set(selected.map((q) => q.id))
    const leftovers = questions.filter((q) => !usedIds.has(q.id))
    const remaining = mix === 'confidence' ? byTierAscending(shuffle(leftovers)) : shuffle(leftovers)
    selected.push(...remaining.slice(0, count - selected.length))
  }

  // A confidence quiz keeps a deliberate easiest-first order so the child opens
  // on questions they are likely to get right. Everything else is shuffled.
  return mix === 'confidence'
    ? byTierAscending(selected).slice(0, count)
...
export interface SelectQuizOptions {
  ...
  /**
   * Tier mix. Default 'balanced'. Pass 'confidence' for a child's first attempt
   * on a topic: sprout-heavy, no lightning, ordered easiest first.
   */
  mix?: QuizMix
}
...
export async function selectQuizQuestions(
  ...
  const [{ recentlySeenIds: recentIds, mistakeIds, recentAnswers }, priors] = await Promise.all([
    getHistoryIds(supabase, profileId, topicId, lookbackAttempts, 3),
    loadDifficultyPriors(supabase),
  ])

  // Estimate the child's ability on this topic so selection can pitch items at
  // the desirable-difficulty sweet spot. No-op for a cold start, or for a pool
  // with neither item calibrations nor group priors.
  const ability = estimateAbility(recentAnswers, allQuestions, priors)
  ...
  // Every branch below keeps a confidence quiz in easiest-first order, so a thin
  // pool degrades to "as easy as we can make it" rather than back to a shuffle.
  // Shuffle first either way, so within-tier variety survives the tier sort
  // (Array.sort is stable, so equal-tier items keep the shuffled order).
  const order = (items: AdaptiveQuestion[]) =>
    mix === 'confidence' ? byTierAscending(shuffle(items)) : shuffle(items)

  if (fresh.length >= count) {
    // Ideal path: enough fresh questions for a full quiz.
    selected = pickWithTierBalance(fresh, count, ability, mix, priors)
    ...
    const fromFresh = fresh.length > 0 ? pickWithTierBalance(fresh, fresh.length, ability, mix, priors) : []
    const fromSeen = order(seen).slice(0, count - fromFresh.length)
    selected = order([...fromFresh, ...fromSeen])
