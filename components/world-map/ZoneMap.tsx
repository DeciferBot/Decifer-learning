...
// Each row of nodes needs ~110px (64px circle + label + padding).
// yPct values cluster into "rows" — count distinct bands to set height.
function canvasHeight(nodes: ZoneNode[]): number {
  if (nodes.length === 0) return 0
  const yValues = nodes.map((n) => n.yPct)
  const sorted = [...new Set(yValues.map((y) => Math.round(y / 15) * 15))].sort((a, b) => a - b)
  const rowCount = sorted.length
  return Math.max(130, rowCount * 110 + 30)
}
...
export function ZoneMap({ zoneId, zoneName, subjectName, theme, subjectColor, nodes, allCompleted, checkpointTopicId }: Props) {
  ...
  return (
    ...
    >
      ...
      {allCompleted && (
        <div
          className="mx-5 mb-4 mt-2 rounded-xl p-4 text-center"
