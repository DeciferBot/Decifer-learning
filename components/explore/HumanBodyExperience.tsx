...
export function HumanBodyExperience({ explorer }: { explorer: BodyExplorer }) {
  ...
  useEffect(() => {
    ...
    return () => {
      ...
      fetch('/api/explore/track', {
        ...
        body: JSON.stringify({ aidType: 'human-body', durationSeconds: duration, askCount: askCountRef.current }),
        keepalive: true,
      }).catch(() => {})
    }
  }, [])
  ...
  const handleExplore = useCallback((topicKey: string) => {
    fetch('/api/explore/track', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ aidType: 'human-body', topicKey }),
    }).catch(() => {})
  }, [])
  ...
  return (
    <div className="fixed inset-0 z-10">
      ...
      <HumanBody explorer={explorer} onAskDecifer={setAskContext} onExplore={handleExplore} />
