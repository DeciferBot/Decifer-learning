    ...
    <div className="grid aspect-[8/3] grid-cols-8 grid-rows-3">
      {Array.from({ length: 24 }, (_, i) => {
        ...
        return (
          <span
            key={i}
            className="flex items-center justify-center text-[clamp(15px,5.4vw,26px)] leading-none"
            style={{ background: dark ? 'var(--game-sq-dark)' : 'var(--game-sq-light)' }}
          >
            {piece && (
              <span
                style={{
                  color: piece.light ? 'var(--game-piece-light)' : 'var(--game-piece-dark)',
                  textShadow: piece.light ? PIECE_RIM_LIGHT : PIECE_RIM_DARK,
                }}
              >
                {piece.glyph}
              </span>
